...
public class RichEditText extends MentionEditText {
     ...
     * 用户at
     */
    private List<TwitterModel> nameList;

    /**
     * 话题
     */
    private List<TwitterModel> topicList;
    ...
    private void resolveDeleteName() {
        ...
        for (int i = 0; i < nameList.size(); i++) { //循环遍历整个输入框的所有字符
            if ((lastPos = getText().toString().indexOf(nameList.get(i).getValue().replace("\b", ""), lastPos)) != -1) {
                if (selectionStart > lastPos && selectionStart <= (lastPos + nameList.get(i).getValue().length())) {
                    nameList.remove(i);
                    return;
                } else {
                    lastPos++;
                ...
                lastPos += (nameList.get(i)).getValue().length();
            }
        }
    }
    ...
    private void resolveDeleteTopic() {
        ...
        for (int i = 0; i < topicList.size(); i++) { //循环遍历整个输入框的所有字符
            if ((lastPos = getText().toString().indexOf(topicList.get(i).getValue(), lastPos)) != -1) {
                if (selectionStart > lastPos && selectionStart <= (lastPos + topicList.get(i).getValue().length())) {
                    topicList.remove(i);
                    return;
                } else {
                    lastPos++;
                ...
                lastPos += (topicList.get(i)).getValue().length();
            }
        }
    }
        ...
        if (topicList != null && topicList.size() > 0) {
            ...
            while (matcher.find()) {
                ...
                for (int i = 0; i < topicList.size(); i++) {
                    TwitterModel topicModel = topicList.get(i);
                    if (topicModel.getValue().equals(mentionText)
        ...
        if (nameList != null && nameList.size() > 0) {
            ...
            while (matcher.find()) {
                ...
                for (int i = 0; i < nameList.size(); i++) {
                    TwitterModel userModel = nameList.get(i);
                    if (userModel.getValue().replace("\b", "").equals(mentionText.replace("\b", ""))
                            && getRangeOfClosestMentionString(start, end) != null) {
        ...
        if (selectionStart > 0) {
            ...
            for (int i = 0; i < nameList.size(); i++) {
                if ((lastPos = getText().toString().indexOf(
                        nameList.get(i).getValue(), lastPos)) != -1) {
                    if (selectionStart >= lastPos && selectionStart <= (lastPos + nameList.get(i).getValue().length())) {
                        setSelection(lastPos + nameList.get(i).getValue().length());
                        success = true;
                    }
                    lastPos += (nameList.get(i)).getValue().length();
                }
            ...
            if (!success && topicList != null) {
                ...
                for (int i = 0; i < topicList.size(); i++) {
                    if ((lastPos = getText().toString().indexOf(
                            topicList.get(i).getValue(), lastPos)) != -1) {
                        if (selectionStart >= lastPos && selectionStart <= (lastPos + topicList.get(i).getValue().length())) {
                            setSelection(lastPos + topicList.get(i).getValue().length());
                        }
                        lastPos += (topicList.get(i)).getValue().length();
                    }
                }
     ...
     * @return Spannable
     */
    private static Spannable resolveTopicInsert(Context context, String text, String color, List<TwitterModel> listTopic) {
        Spannable spannable;
        if (listTopic != null && listTopic.size() > 0) {
            Map<String, String> topics = new HashMap<>();
            for (TwitterModel topicModel : listTopic) {
                topics.put(topicModel.getValue(), topicModel.getValue());
            }
            //查找##
     ...
     * @return Spannable
     */
    private Spannable resolveAtInsert(String text, Spannable spannable, String color, List<TwitterModel> listUser) {
        ...
        if (listUser.size() > 0) {
            for (TwitterModel userModel : listUser) {
                names.put(userModel.getValue(), userModel.getValue());
            }
        }
     ...
     * @param topicList 话题
     */
    public void setModelList(List<TwitterModel> nameList, List<TwitterModel> topicList) {
    ...
    public void setRichEditTopicList(List<TwitterModel> list) {
        if (list != null) {
            this.topicList = list;
        }
    }

    public void setRichEditNameList(List<TwitterModel> list) {
        if (list != null) {
            this.nameList = list;
        }
    }
     ...
     */
    public void resolveText(TwitterModel userModel) {
        String userName = userModel.getValue();
        userModel.setValue(userName + "\b");
     ...
     */
    public void resolveTopicText(TwitterModel topicModel) {
        topicList.add(topicModel);
        int index = getSelectionStart();
        SpannableStringBuilder spannableStringBuilder =
                new SpannableStringBuilder(getText());
        //直接用span会导致后面没文字的时候新输入的一起变色
        Spanned htmlText = Html.fromHtml(String.format("<font color='%s'>" + topicModel.getValue() + "</font>", colorTopic));
        spannableStringBuilder.insert(index, htmlText);
        setText(spannableStringBuilder);
     ...
     */
    public void resolveInsertText(Context context, String text, List<TwitterModel> listUser, List<TwitterModel> listTopic) {
     ...
     */
    public void resolveTopicResult(TwitterModel topicModel) {
        resolveTopicText(getResolveModel(topicModel));
    }
     ...
     * @param topicModel 话题model
     */
    public void resolveTopicResultByEnter(TwitterModel topicModel) {
        deleteByEnter = true;
        ...
        resolveTopicText(getRealModel(topicModel));
     ...
     */
    public void resolveAtResult(TwitterModel userModel) {
        resolveText(TwitterModel.resolveModel(userModel));
    }
     ...
     * @param userModel       用户model
     */
    public void resolveAtResultByEnterAt(TwitterModel userModel) {
        if (getSelectionEnd() == 0) {
            getText().delete(0, 1);
        } else {
            int index = getText().toString().indexOf("@", getSelectionEnd() - 1);
     ...
     * @return
     */
    public List<TwitterModel> getRealUserList() {
        List<TwitterModel> list = new ArrayList<>();
        if (nameList == null) {
            return list;
        }
        for (TwitterModel userModel : nameList) {
            list.add(TwitterModel.realModel(userModel));
     ...
     * @return
     */
    public List<TwitterModel> getRealTopicList() {
        List<TwitterModel> list = new ArrayList<>();
        if (topicList == null) {
            return list;
        }
        for (TwitterModel topicModel : topicList) {
            list.add(TwitterModel.realModel(topicModel));
        }
        return list;
    ...
    public TwitterModel getResolveModel(TwitterModel model) {
        return TwitterModel.resolveModel(model);
    }

    public TwitterModel getRealModel(TwitterModel model) {
        return TwitterModel.realModel(model);
    }
}
